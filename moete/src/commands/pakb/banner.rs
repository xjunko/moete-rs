...
pub async fn banner_rotate(
    ...
    loop {
        ...
        let current_index: i32 =
            (current_day_of_year + custom_offset) % SERVER_THEME.len() as i32;
        ...
        let icon_attachment = {
            if let Some(logo_url) =
                moete_discord::cdn::get_cdn_url(logo, config.clone()).await
            {
        ...
        let banner_attachment: Option<String> = {
            if let Some(banner_url) =
                moete_discord::cdn::get_cdn_url(banner, config.clone()).await
            {
