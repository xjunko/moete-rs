...
pub async fn banner_rotate(ctx: Arc<serenity::Context>, config: Arc<core::Config>) {
    loop {
        ...
        let icon_attachment = {
            if let Some(logo_url) = builtins::discord::cdn::get_cdn_url(logo, config.clone()).await
            {
        ...
        let banner_attachment: Option<String> = {
            if let Some(banner_url) =
                builtins::discord::cdn::get_cdn_url(banner, config.clone()).await
            {
                builtins::discord::cdn::to_base64(&banner_url).await
            } else {
                None
            }
        };
